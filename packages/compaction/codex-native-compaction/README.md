...
For a route whose provider is not `codex`, this class delegates to `BasicCompactionEngine.summarize()`. Mixed-provider deployments can therefore use one backend while retaining portable text checkpoints for their non-Codex routes. Codex-native compaction itself always uses the conversation's routed Codex model rather than `summarizationProvider` or `summarizationModel`, because provider state must be produced by the backend that will consume it.
...
`summarizationProvider`, `summarizationModel`, and `maxTokens` still apply to the basic fallback used by non-Codex routes. They do not alter a Codex-native request; the Codex adapter owns native compaction output limits and protocol details.
...
Native compaction adds one Codex request over the selected prefix and its provider-defined opaque output. `maxTokens` applies only to the non-Codex text fallback.
