...
当路由提供方不是 `codex` 时，该类委托给 `BasicCompactionEngine.summarize()`。因此混合提供方部署可以使用同一个后端，而非 Codex 路由仍获得可移植的文本检查点。Codex 原生压缩始终使用会话已路由的 Codex 模型，不使用 `summarizationProvider` 或 `summarizationModel`，因为该提供方状态必须由随后消费它的后端生成。
...
`summarizationProvider`、`summarizationModel` 与 `maxTokens` 仍作用于非 Codex 路由使用的基础回退；它们不会改变 Codex 原生请求。原生压缩的输出限制与协议细节由 Codex 适配器拥有。
...
原生压缩会增加一次覆盖选中前缀的 Codex 请求及其由提供方定义的不透明输出。`maxTokens` 只作用于非 Codex 文本回退。
