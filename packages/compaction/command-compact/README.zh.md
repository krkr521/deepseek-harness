...
`dsh-command-compact` 为聊天 UI 添加 `/compact` 命令：输入它，对话就会按需压缩——即使尚未触发自动压力，较早历史也会被替换为一条摘要。追加提供方与模型即可为本次调用选择摘要模型，而不改变对话模型。在随附 Web GUI 中，裸命令会打开可搜索的选择器，显示确切提供方／模型 id，再提交该参数形式。该命令适用于任何压缩后端，且不消耗模型轮次；完成后你会看到压缩了多少历史项以及估算节省的 token 数。当 agent 正在轮次中或压缩已在运行时，它会告诉你压缩暂不可用。运行期间你发送的提示词会保持排队，并在压缩结束后才开始。
...
当对话已经很长、想立即压缩时，在聊天 UI 中输入 `/compact`。随附 Web 组合会为裸命令打开提供方／模型选择器；选择一行会用目录中的确切 id 提交 `/compact <provider> <model>`。当本次摘要应使用不同于已配置目标或当前对话路由的模型时，也可以直接输入该参数形式。其他命令适配器保留裸零参数形式，并使用后端配置。随附 `dsh` 基础配置把该命令挂载在默认后端旁，因此它通常已经可用。

### 使用命令

| 输入 | 结果 |
|---|---|
| 随附 Web GUI 中的裸 `/compact` | 打开可搜索的提供方／模型选择器；选择一行会用该确切路由压缩一段有效、平衡的较早范围。 |
| 直接命令适配器中的 `/compact` | 即使未达到自动压力，也使用后端路由压缩一段有效、平衡的较早范围，然后报告被替换的历史项数量与估算 token 数。 |
| `/compact codex gpt-5.6-luna` | 仅本次使用 `codex/gpt-5.6-luna` 压缩；后续对话请求保留原有路由。 |
| `/compact`，但没有可压缩历史 | `No compactable history yet.`——不会有任何改变。 |
| 除零个或两个参数之外的输入 | `Usage: /compact [<provider> <model>]`——提供方与模型必须成对给出。 |
...
- **与后端无关的控制。** 处理器只依赖 `compactNow(agent, signal, options)`，因此可与任何 `CompactionEngine` 实现协作。可选的 `summarizationTarget` 仅作用一次；调用该命令的 agent（智能体）仍是确切的对话目标，发起分发的 UI 会通过 seam 转发取消信号。
...
| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | 插件入口：`/compact` 注册、提供方／模型解析、参数拒绝、错误码映射、生命周期排空 |
...
- [Web 模型选择](../../client/ui-model-selection/README.zh.md)——装饰 Web 裸调用的目录驱动选择器。
...
命令生命周期不会增加模型 token。成功压缩会用一份带框架的摘要替换所选范围，从而减少后续请求；摘要生成本身需要一次辅助请求，并使用本次所选路由或后端默认路由。该选择不会改变后续对话请求。
...
- **不接受范围或保留参数**——参数形式只选择本次摘要的提供方／模型。显式范围仍由编程接口 `compactRegion()` 处理，保留设置仍属于后端配置。
- **Web 选择器仅提供建议**——它只列出已公布目录行；直接输入参数仍可指定宿主服务但未公布的确切路由。
...
- **范围与保留参数，尚未决定**——共享语法只拥有可选的提供方／模型对；范围与保留控制仍属于编程接口或部署配置。
