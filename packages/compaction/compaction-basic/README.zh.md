...
`dsh-compaction-basic` 让长时 agent 会话在接近模型上下文上限时仍能正常工作。token 压力上升时，它会自动把对话最旧的部分压缩为摘要，并保持近期部分完整；上下文溢出错误发生后，它会压缩并重试。你也可以通过 `dsh-command-compact` 的 `/compact` 按需压缩，并为本次调用选择摘要提供方／模型，还可挂载 `dsh-compaction-tool-result-pruner` 先修剪超大工具输出。压缩的代价是一次额外的模型请求，它读取所选历史并写出摘要；只有摘要文本会被保留。它只压缩派生历史——无法缩减系统提示词、工具或会话前缀，也无法拆分单个不可分单元（例如一次超大工具调用）。
...
挂载 `dsh-command-compact` 后，在聊天 UI 中输入 `/compact` 即可立即压缩，即使未达到压力阈值。使用 `/compact <provider> <model>` 可只让本次摘要经过另一个模型；保留策略与 `maxTokens` 仍来自当前对话的压缩策略，后续对话请求也保留原有路由。命令会报告压缩了多少历史项以及估算节省的 token 数。当 agent 正在轮次中或压缩已在运行时，`/compact` 会报告压缩暂不可用；运行期间你发送的提示词会被接受，并在压缩结束后才开始。
...
- **`summarize()` 是唯一的子类钩子。** 基于模板或远程摘要器的子类可以覆盖它，并接收可选的本次摘要目标；压力、保留、被引用的源事件、缩减验证与已遮蔽 token 计量仍由 token meter 负责。
...
直接 `ctx.llm.stream()` 调用依次使用手动调用的本次提供方／模型对、已配置的对、最新已记录请求目标与 `AgentOptions` 对，而不运行仅用于 agent loop 的 `agent/request` 扩展点。本次选择只改变路由；输出上限与其他策略字段仍根据对话路由选择。该调用逐字回放会话自身的系统提示词、工具与已遮蔽区域消息——包括所选适配器必须解析或明确拒绝的图片引用——并将压缩指令作为最后一条 user 消息追加，从而复用提供方的热前缀 cache，而非使它失效。调用将 `GenerateOptions.purpose` 设为 `compaction`；只有返回文本进入检查点，推理与工具调用都会被排除。图片输出会以 `UNSUPPORTED_CONTENT` 失败，而不是消失。替换 user 消息用 `<compacted-summary>` 标签框定摘要；原始摘要保留在 `compaction/summary` 事件上。
