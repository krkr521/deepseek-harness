...
Once the Workspace list baseline is ready, browser-persisted expansion and Session-order records retain only current presentation keys plus Ungrouped and the flat-list account. Real Workspaces initialize from `WorkspaceView.sessionIds`, while Ungrouped and the cross-Workspace flat list initialize from recency. A `workspacePresentation` registration may replace matching real Workspaces with one display collection; Session summaries with a matching retained `cwd` also join that collection, even if their old Workspace directory is unavailable. Collection headers have no rename, delete, or Workspace-drag actions, and their Session ordering stays browser-local because no single Host Workspace owns the combined account. New Session resolves the collection's current real Workspace before creation. The shared sidebar projection hides rows whose durable Session summary has `origin: 'subagent'`, and each visible ordinary row inherits the blue activity indicator while any descendant reached through uninterrupted subagent-origin lineage is running. The same pure derivation reads the Schedule key from list projection values for grouped, flat, and search nodes; the package uses only the type-only `@deepseek-ai/dsh-schedule/client` dependency and does not import the Schedule runtime or `ui-schedule`.
