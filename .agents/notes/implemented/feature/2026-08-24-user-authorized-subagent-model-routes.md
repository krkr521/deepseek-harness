...
The ACP setting owner requires the Host settings service and waits for its initial stored value before publishing model selection. The ACP bridge in turn requires that published service before accepting a Session. This startup dependency prevents an early `session/new` from sampling the composition default while the stored allowlist is still loading. Per-Agent tool definitions belong to both the Agent and the settings-dependent composition, so either owner's teardown waits for their removal; reloading preserves the recorded Session policy.
