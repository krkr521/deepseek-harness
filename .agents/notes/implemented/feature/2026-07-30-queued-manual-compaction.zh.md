...
自动压缩（compaction）可以保护上下文窗口，但交互用户还需要一种确定性方法，在压力策略触发前压缩累积的历史。用户可能需要一个更便宜或当前可用的摘要模型，同时不改变后续对话轮次使用的模型。把 `/compact` 作为提示词文本发送会消耗一个模型轮次，还会让会话模型重新解释一项直接控制操作。在某个 UI 内实现该功能，则会重复命令发现、生命周期日志记录、取消与后端策略。
...
`@deepseek-ai/dsh-command-compact` 通过 `ctx.commands` 注册 `/compact [<provider> <model>]`。零个参数保留后端配置与对话路由；恰好两个参数只为本次摘要选择提供方／模型。命令调用第三个抽象 `CompactionEngine` 操作 `compactNow(agent, signal, options?)`，并把封闭的 `ManualCompactionError` 分类体系（`busy | changed | summary | commit | persistence`）映射为直接 UI 结果。`command/run` 和 `command/done` 保留命令生命周期，同时不进入模型历史，也不消耗模型循环轮次。

Web 模型选择插件只为裸 `/compact` 调用装饰一个基于会话模型目录的弹窗。每行显示模型名称与确切 `provider/model` id；选择一行会经 `session.command` 提交完整命令。带参数的 `/compact <provider> <model>` 路径仍由宿主持有，选择器也不会调用 `session.selectModel`，因此不能改变对话路由。目录仅提供建议：选择器能避免已公布行的 id 转录错误，而直接输入仍可指定宿主服务但未公布的路由。
...
该 seam 的 `ManualCompactAgentContext` 只在压缩已需使用的会话与路由事实之上增加 `runMaintenance()`。`ManualCompactionOptions` 携带可选命令来源与 `CompactionSummarizationTarget`；每个后端都必须把该目标限定在当前摘要，并保持 agent 的对话路由不变。保留、平衡、摘要、标记排序、替换与持久性仍由后端负责。
...
命令包固定注册与输入提示、Loader 组合、零个或两个参数的解析、精确目标与来源转发、精确的成功／失败文本、取消、不进入模型历史的保证，以及 dispose 在中止使执行器停止等待处理器后，仍会跨越相互独立的闭合与 flush 边界等待该处理器结算。基础后端测试固定本次目标传递到子类钩子、省略时的默认路由、实际的一次性 LLM 分发，以及保留由对话选择的策略字段。浏览器插件测试固定确切提供方／模型行详情、不进行持久模型选择的命令提交、拒绝路径、已寻址会话隐藏与装饰项释放。Web 组合快照从真实斜杠菜单打开选择器，记录其可搜索的确切 id 行，并保留带参数的 composer 语法。客户端运行时投影测试固定 end-seed 中断，以及随后一次独立尝试的完成。`queued-manual-compact` 终端快照通过已组装 TUI 驱动真实按键：`/help` 可发现该命令；被暂停的摘要会接纳一个排队提示词和即时注入；`turn: null` 标记与 flush 先于排队提示词轮次；命令生命周期保持纯日志；派生顺序固定为检查点 → 注入 → 排队提示词。

## 后果

交互用户无需消耗会话模型轮次即可压缩有效历史，并可为辅助摘要选择另一个模型，而不改变后续对话请求。Web 用户可以选择已公布的确切路由，无需转录提供方与模型 id；终端和直接输入用户保留相同的显式命令语法。在命令前获接纳的提示词胜出；命令期间提交的提示词会以原有队列身份等待。手动压缩会消耗会话 seq，但不消耗轮次编号。
